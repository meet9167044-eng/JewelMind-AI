...
print("=== Phase 9 Ground Truth (June 2026 vs May 2026, business_id=1) ===")
print(f"May  rows  : {len(may):,}  | W_A = {W_A:,.2f}g | GP_A = Rs {GP_A:,.2f}")
print(f"June rows  : {len(june):,}  | W_B = {W_B:,.2f}g | GP_B = Rs {GP_B:,.2f}")
print("")
print(f"delta_GP    = Rs {delta_GP:>12,.2f}")
print(f"delta_vol   = Rs {delta_vol:>12,.2f}")
print(f"delta_disc  = Rs {delta_disc:>12,.2f}")
print(f"delta_mc    = Rs {delta_mc:>12,.2f}")
print(f"delta_mix   = Rs {delta_mix:>12,.2f}")
print(f"delta_metal = Rs {delta_metal:>12,.2f}")
print("")
check = delta_vol + delta_disc + delta_mc + delta_mix + delta_metal
print(f"Sum of drivers = Rs {check:,.2f}  (must equal delta_GP = Rs {delta_GP:,.2f})")
